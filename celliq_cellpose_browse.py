...
def process_group(group):
    """Create a Dask array from a grouped subset of the dataframe."""
    index_map, index_map_reverse, dask_array = create_dask_array(group)
    return index_map, index_map_reverse, dask_array

def parallel_dask_array(df):
    # Group by wells or another meaningful dimension
    grouped = df.groupby(by=[PLATE, WELLSITE])
    
    # Convert to a dictionary {group_key: group_df} for parallel processing
    group_dict = {name: group for name, group in grouped}
    
    # Process each group in parallel using Dask's delayed computation
    results = [dask.delayed(process_group)(group) for group in group_dict.values()]
    
    # Compute everything in parallel
    computed_results = dask.compute(*results, num_workers=2)
    
    # Extract results
    index_maps, index_maps_reverse, dask_arrays = zip(*computed_results)
    
    # Combine Dask arrays along the appropriate axis
    final_dask_array = da.concatenate(dask_arrays, axis=1)  # Adjust axis as needed
    print(f'parallel_dask_array.shape {final_dask_array.shape}')
    ...
    for im in index_maps:
        for plate, wellsite in im.keys():
            # here 
            if not wellsite in wellsites:
                wellsites.append(wellsite)
            plate_idx = 0 # here we have just one plate at a time
            wellsite_idx = len(wellsites) - 1
            
            index_map[(plate,wellsite)] = (plate_idx, wellsite_idx)
            index_map_reverse[(plate_idx, wellsite_idx)] = (plate, wellsite)
